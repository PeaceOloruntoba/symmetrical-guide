<nav class="bg-white shadow py-3">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        <div class="flex items-center">
            <a href="{{ route('home') }}" class="text-xl font-semibold text-gray-800">{{ config('app.name', 'Laravel') }}</a>
        </div>

        <div class="flex items-center">
            <form action="{{ route('search.products') }}" method="GET" class="relative rounded-full overflow-hidden border border-green-500">
                <input
                    type="search"
                    name="search"
            ...
            <div class="ml-4 relative">
                <button id="categoryDropdownButton" data-dropdown-toggle="categoryDropdown" class="inline-flex items-center text-gray-700 hover:text-green-500 font-medium">
                    Alle Kategorien
                    <svg class="h-5 w-5 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                </button>
                <div id="categoryDropdown" class="z-10 hidden bg-white divide-y divide-gray-100 shadow w-44 dark:bg-gray-700">
                    <ul class="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="categoryDropdownButton">
                        @foreach ($categories as $category)
                            <li>
                                <a href="{{ route('categories.show', $category->id) }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{{ $category->name }}</a>
                            </li>
                        @endforeach
                        <li>
                            <a href="{{ route('categories.index') }}" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Alle Anzeigen</a>
                        </li>
                    </ul>
                </div>
            </div>
